...
* Use nvm to set up Node 24.x via `nvm use lts/krypton`.
* Map the host name `mongo` to `127.0.0.1` using `/etc/hosts`.
* Fire up a local MongoDB instance using Docker: 

```docker run --ulimit nofile=64000:64000 --name mongo -p 127.0.0.1:27017:27017 mongo:8.3.8 mongod --oplogSize 128 --replSet rs0 --storageEngine=wiredTiger```
